...
public class BoardController {
	...
	public int createPost(String title, String content, int writer) {
	...
	public ArrayList<Board> serchForUser(int writer) {
		ArrayList<Board> list = new ArrayList<>();
		list = new BoardService().searchForUser(writer);
		if(list.isEmpty()) {
			new BoardView().serviceFail(writer + "가 작성한 글이 없습니다.");	
		}else {
			new BoardView().showList(list,writer);
		}
